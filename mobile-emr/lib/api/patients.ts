...
// Base URL — swap for real backend in production
...
export const patientsApi = {
  list: async (params?: { page?: number; pageSize?: number; search?: string; status?: string }) => {
    ...
    if (params?.pageSize != null) query.set('_per_page', String(params.pageSize));
    if (params?.search) query.set('q', params.search);
    if (params?.status) query.set('status', params.status);
    const qs = query.toString();
    const payload = await request<PatientsResponse | JsonServerPaginatedResponse | Patient[]>(`/patients${qs ? `?${qs}` : ''}`);
